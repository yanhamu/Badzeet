...
public class ApiActionMethodConvention : IActionModelConvention
{
    public void Apply(ActionModel action)
    {
        ...
        {
            if (action.Parameters.Any(p => p.Name == "accountId")) action.Filters.Add(new AccountIdFilter());

            if (action.Parameters.Any(p => p.Name == "budgetId")) action.Filters.Add(new BudgetIdFilter());

            if (action.Parameters.Any(p => p.Name == "paymentId")) action.Filters.Add(new PaymentIdFilter());
        }
    }
}